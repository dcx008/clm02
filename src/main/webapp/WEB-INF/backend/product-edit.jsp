...
<article class="page-container">
    <form class="form form-horizontal"  action="toupdate">
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>产品ID：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="${product.productId}" placeholder="" readonly="readonly" id="articletitle" name="productId">
            </div>
        </div>
        <div class="row cl">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select name="category" class="select">
                    ...
                    <option value="头盘冷菜" <c:if test="${product.category=='头盘冷菜'}">selected="selected"</c:if>>头盘冷菜</option>
                    <option value="煲煲共享" <c:if test="${product.category=='煲煲共享'}">selected="selected"</c:if>>煲煲共享</option>
                    <option value="私房美味" <c:if test="${product.category=='私房美味'}">selected="selected"</c:if>>私房美味</option>
                    <option value="招牌菜" <c:if test="${product.category=='招牌菜'}">selected="selected"</c:if>>招牌菜</option>
                    <option value="家常美味" <c:if test="${product.category=='家常美味'}">selected="selected"</c:if>>家常美味</option>
                    <option value="田园时蔬" <c:if test="${product.category=='田园时蔬'}">selected="selected"</c:if>>田园时蔬</option>
                    <option value="西式快餐" <c:if test="${product.category=='西式快餐'}">selected="selected"</c:if>>西式快餐</option>
                    <option value="美味靓汤" <c:if test="${product.category=='美味靓汤'}">selected="selected"</c:if>>美味靓汤</option>
                    <option value="精美点心" <c:if test="${product.category=='精美点心'}">selected="selected"</c:if>>精美点心</option>
                    <option value="米饭" <c:if test="${product.category=='米饭'}">selected="selected"</c:if>>米饭</option>
